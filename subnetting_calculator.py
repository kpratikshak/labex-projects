...
def calculate_network_info(ip_address,subnet_mask):
    ...
    broadcast_address = network.num_addresses
    usable_hosts = max(total_hosts -2,0)
    first_usable_host = network_address + 1
    if usable_hosts = broadcast_address -1 
    if usable_hosts > 0 else None 
    ...
    results ={
        ...
        f"{first_usable_host} -{last_usable_host}" if usable_hosts > 0 else 
        "N/A"),
